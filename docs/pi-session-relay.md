...
等待上限与催单来自飞书配置 `askTimeoutSec` / `askNotifySec`（秒，默认都是 0，即永久等待、不催单）：写在配置文件里，或经 `OMPFEISHU_ASK_TIMEOUT_SEC` / `OMPFEISHU_ASK_NOTIFY_SEC`（pi 侧前缀为 `FEISHU_`）与 `/config` 设置。超时按原生 `ask` 的语义处理：未答的问题选推荐项，没有推荐就选第一个，卡片标注「超时自动作答」；终端侧对话框按同一上限收尾。终端 `ask.timeout` 属于宿主设置，扩展读不到，所以飞书侧的等待上限以这里的配置为准。

结果以普通工具结果返回，文本与 `details` 与原生 `ask` 同构（单问题扁平字段、多问题 `results` 数组，含 `selectedOptions`、`customInput`、`note`、`timedOut`），模型不需要区分答案来自哪一侧。
